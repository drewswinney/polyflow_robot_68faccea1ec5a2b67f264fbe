...
let
  user      = "admin";
  password  = "password";
  hostname  = "68faccea1ec5a2b67f264fbe";  # keep your template if you render it; otherwise set a real name
...
in
{
  ...
  # 1) Setup service: clone/pull and build the workspace with colcon
  systemd.services.polyflow-setup = {
    ...
    script = ''
      ...
      cd "${wsDir}"
      # You can scope to specific packages if desired:
  ...
  # 2) Runtime service: run your launch file via `ros2 launch`
  #
  # NOTE: We source the workspace's install/setup.bash so that ROS can discover
  #       your *colcon-built* package (since it's not packaged by Nix).
  systemd.services.polyflow-webrtc = {
    ...
    # Only need `ros2` binary on PATH; launch plugin is in closure via systemPackages
    ...
    serviceConfig = {
      ...
      StandardOutput = "journal";
      StandardError  = "journal";
      # If your node needs GPU/tty, you can add binds/TTY options as needed.
    };

    # Shell script so we can `source` the colcon overlay
    script = ''
      set -euo pipefail
